    ...
    const analyzed: SmartImportRow[] = rows.map((raw, idx) => {
      ...
      if (type === 'tasks') {
        ...
        mapped = {
          ...
          plannedStartDate: date(pick(raw, ['תאריך התחלה','planned start','start date'])) || new Date().toISOString().slice(0,10),
          plannedEndDate: date(pick(raw, ['תאריך סיום מתוכנן','planned end','end date'])),
          deadline: date(pick(raw, ['דדליין','תאריך יעד','deadline'])),
